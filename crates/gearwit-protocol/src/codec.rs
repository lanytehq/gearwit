...
use crate::messages::{WaiterLink, WaiterLinkError, parse_waiter_link};
...
pub fn decode_frame(bytes: &[u8]) -> Result<WaiterLink, FrameError> {
    ...
    let json = std::str::from_utf8(&bytes[4..need]).map_err(|_| FrameError::Truncated)?;
    parse_waiter_link(json).map_err(FrameError::Message)
}
